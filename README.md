...
**대응 및 판단**: 풀 크기를 50으로 확장하자 50 VU 스모크 테스트는 정상화됐지만, 500 VU 단계에서 다시 포화됐습니다. 풀 크기 조정은 임계점을 늦출 뿐 근본 해결은 아니라고 판단했으며, DM 저장 로직의 DB 왕복 횟수 축소와 Redis Streams 기반 전송·비동기 영속화 분리, 백프레셔 도입을 후속 개선 방향으로 도출했습니다.
