...
| 실시간 알림     | SSE + Redis Stream           | 단방향 알림, 재연결 시 미수신 이벤트 보정 |
...
| **Redis Stream** | **ACK·PEL 지원**  | **기존 Redis 활용, Consumer Group 지원** | **PEL 복구 정책을 직접 관리해야 함**   |

> 여기서 ACK는 최종 사용자의 메시지 수신 확인이 아니라, Consumer의 처리 완료 확인을 의미합니다.

Redis Stream은 기존 Redis 인프라를 활용하면서 Consumer Group·ACK·PEL(Pending Entries List)을 제공해 선택했습니다. 이를 통해 **Stream에 발행된 이후 Consumer 처리에 실패한 메시지**를 PEL에서 조회하고 재처리할 수 있도록 구성했습니다.
...
### 3. [개인 고도화] 동시 다발 메시지 전송 시 DB 커넥션 풀 고갈
...
**대응 및 판단**: 풀 크기를 50으로 확장한 뒤 같은 50 VU 조건으로 재검증하자 타임아웃이 해소됐지만, 500 VU 단계에서 다시 포화됐습니다. 풀 크기 조정은 임계점을 늦출 뿐 근본 해결은 아니라고 판단했습니다. 구체적인 개선 방향은 [향후 개선 사항](#-향후-개선-사항)에 정리했습니다.

---

### 4. [개인 고도화] SSE 재연결 시 알림 유실
...
| Redis Stream Consumer 순수 처리량      | 약 6,300 msg/sec (서비스 경로에서 관측한 순간 최대 312 msg/sec 대비 약 20배)                    |
...
부하테스트 중 발생한 병목은 k6 지표와 서버 스레드 덤프(`jcmd Thread.print`)를 함께 분석했습니다. 3,000 VU 이후 서버가 일시 무응답에 빠졌을 때 처리 스레드 216개(Tomcat 200 + WebSocket inbound 16)가 모두 `HikariPool.getConnection()`에서 대기 중이었고, `BLOCKED` 상태의 상호 대기 스레드는 없었습니다. 이를 통해 데드락보다 DB 커넥션 대기로 처리 슬롯이 점유된 상태가 핵심임을 확인했습니다. 다만 executor 작업 큐의 포화 여부와 명시적인 작업 거부(`RejectedExecutionException`) 발생 여부는 별도로 측정하지 않았습니다.
...
* PgBouncer 등 커넥션 풀러 검토 — 연결 관리 비용과 대기 시간을 줄일 수 있는지 재측정
